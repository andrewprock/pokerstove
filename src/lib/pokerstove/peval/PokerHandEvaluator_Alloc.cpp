...
boost::shared_ptr<PokerHandEvaluator>
PokerHandEvaluator::alloc(const string& strid)
{
    ...
    switch (strid[0])
    {
        ...
        case 'l':  //     lowball (A-5)
            ret.reset(new UniversalHandEvaluator(
                1, 5, 0, 0, 0, &CardSet::evaluateLowA5, NULL));
            break;

        case '3':  //     three card poker
            ret.reset(new UniversalHandEvaluator(1, 3, 0, 0, 0,
                                                 &CardSet::evaluate3CP, NULL));
            break;
        ...
        case 'P':
            if (strid[2] == 'h' || strid[2] == 'H')  // plh/PLH
                ...
                throw std::runtime_error(
                    "no comatible pot limit game available");
            break;
        ...
        case 'q':  //     stud high/low no qualifier
            ret.reset(new UniversalHandEvaluator(1, 7, 0, 0, 0,
                                                 &CardSet::evaluateHigh,
                                                 &CardSet::evaluateLowA5));
            break;
